package com.example.jooff.shuyi.constant;

/**
 * Created by Jooff on 2017/8/2.
 * Tomorrow is a nice day
 */

public final class SettingDefault {

    public static final int THEME_ID = ThemeColor.BLUE;

    private SettingDefault() {

    }

    public static final String COLOR_PRIMARY = "#1b82d2";

    public static final String COLOR_PRIMARY_DARK = "#14619d";

    public static final int TRANS_FROM = TransSource.FROM_YOUDAO;

    public static final String RESULT_LAN = "zh";

}
